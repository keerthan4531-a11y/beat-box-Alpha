...
fun NavGraphBuilder.navigationBuilder(
    ...
    composable(Screens.Beats.route) {
        iad1tya.echo.music.ui.screens.beats.BeatsScreen(navController)
    }
    ...
    composable(Screens.Profile.route) {
        iad1tya.echo.music.ui.screens.profile.ProfileScreen(navController)
    }

    composable(Screens.AiAssistant.route) {
        val playerConnection = iad1tya.echo.music.LocalPlayerConnection.current
        com.music.echo.ui.screens.VoiceAssistantScreen(
            onPlayCommand = { query ->
                if (playerConnection != null) {
                    val scope = kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.IO)
                    scope.launch {
                        try {
                            val result = com.music.innertube.YouTube.search(query, com.music.innertube.YouTube.SearchFilter.FILTER_SONG).getOrNull()
                            val firstSong = result?.items?.firstOrNull() as? com.music.innertube.models.SongItem
                            if (firstSong != null) {
                                withContext(kotlinx.coroutines.Dispatchers.Main) {
                                    val metadata = firstSong.toMediaMetadata()
                                    val endpoint = firstSong.endpoint ?: com.music.innertube.models.WatchEndpoint(videoId = firstSong.id)
                                    playerConnection.playQueue(iad1tya.echo.music.playback.queues.YouTubeQueue(endpoint, metadata))
                                }
                            }
                        } catch (e: Exception) {
                            e.printStackTrace()
